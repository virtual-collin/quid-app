 ...
 * quick stat cards, and a live transaction history.
 *
 * Transaction history is passed down from App.jsx and updates
 * in real time after every withdrawal. When no transactions have
 * been made yet, a helpful empty state is shown instead.
 */
 ...
 * Shows "Today" or "Yesterday" for recent transactions,
 * falls back to a short date format for older ones.
 *
 * @param {Date} date
 * @returns {string}
 */
function formatTransactionDate(date) {
  ...
  if (txnDate.getTime() === yesterday.getTime()) {
            ...
            {transactions.map(txn => (
              <li key={txn.id} className="dashboard__txn">
                ...
                <div className="dashboard__txn-info">
                  <span className="dashboard__txn-name">
                    Cash withdrawal
                    {txn.isOverdrawn && (
                      <span className="dashboard__txn-overdrawn-badge">
                        Overdrawn
                      </span>
                    )}
                  </span>
                  <span className="dashboard__txn-date">
                    {formatTransactionDate(txn.date)}
                  </span>
                </div>
                <span className="dashboard__txn-amount dashboard__txn-amount--negative">
